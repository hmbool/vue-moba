							...
							<div class="skills bg-white mt-4">
								...
								<div v-if="currentSkill">
									<div class="d-flex pt-4 pb-3">
										...
										<span class="text-gray ml-4">
											(冷却值:
											{{ currentSkill.delay }} 消耗:
											{{ currentSkill.cost }})
										</span>
									</div>
						...
						>
							<div class="d-flex">
								<div
									...
									v-for="item in model.items3"
									:key="item.name"
								>
									<img :src="item.icon" class="icon" />
									<div class="px-2 m-0 flex-1">
										<div>{{ item.name }}</div>
										<div class="fs-xs">
											{{ item.description }}
										</div>
...
export default {
	...
	computed: {
		currentSkill() {
			return this.model.skills[this.currentSkillIndex] // 按照技能改变计算
		},
	},
	methods: {
		async fetch() {
